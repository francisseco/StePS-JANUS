...
#ifndef PERIODIC
cudaError_t forces_cuda(REAL*x, REAL*F, int n_GPU, int ID_min, int ID_max, int JanusAcc);//JANUS
#else
...
{
	forces_cuda(x, F, n_GPU, ID_min, ID_max,JanusAcc);//JANUS
	return;
}
#endif
#ifdef PERIODIC
...
//JANUS
#ifndef PERIODIC
__global__ void ForceKernel(int n, const int N, const REAL *xx, const REAL *xy, const REAL *xz, REAL *F, const REAL* M, const REAL* SOFT_LENGTH, const REAL mass_in_unit_sphere, const REAL DE, const int COSMOLOGY, const int COMOVING_INTEGRATION, int ID_min, int ID_max, int JanusAcc)
{
	...
	for (i = (ID_min+id); i<=ID_max; i+=n)
		{
			for (j = 0; j<N; j++)
			{
				...
				//JANUS - if i mass <0 then reverse sign of aceleration, to avoid runaway effect
				if (M[j]<0){
					wij= -wij;
				}
...
#ifndef PERIODIC
cudaError_t forces_cuda(REAL*x, REAL*F, int n_GPU, int ID_min, int ID_max, int JanusAcc) //Force calculation on GPU - JANUS
{
	int i, j;
	int mprocessors;
	int GPU_ID, nthreads;
...
#pragma omp parallel default(shared) private(GPU_ID, F_tmp, i, j, mprocessors, cudaStatus, N_GPU, GPU_index_min, nthreads, dev_xx, dev_xy, dev_xz, dev_M, dev_SOFT_LENGTH, dev_F)
{
		...
		ForceKernel<<<32*mprocessors, BLOCKSIZE>>>(32 * mprocessors * BLOCKSIZE, N, dev_xx, dev_xy, dev_xz, dev_F, dev_M, dev_SOFT_LENGTH, mass_in_unit_sphere, DE, COSMOLOGY, COMOVING_INTEGRATION, GPU_index_min, GPU_index_min+N_GPU-1,JanusAcc);//JANUS
